...
void main() {
  ...
  group('fetchPastTransactionModel', () {
    test('returns PastTransactionModel when http response is successful',
        () async {
    ...
    test('return error message when http response is unsuccessful', () async {
      ...
      expect(
          () async => await ApiService().fetchNewsModel("/0/1", mockHTTPClient),
          throwsException);
    });
  });
}
